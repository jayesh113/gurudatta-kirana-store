...
function initApp() {
  ...
  // Check saved admin state (safely wrapped for Safari private mode)
  try {
    if (window.sessionStorage && sessionStorage.getItem('gk_is_admin') === 'true') {
      isAdmin = true;
      updateAdminUI();
    }
  } catch (e) {
...
// Avoid mobile race condition if DOMContentLoaded fired before script parsed
...
async function loadProducts() {
  ...
  try {
    ...
    if (countLabel && allProducts.length === 0) {
      countLabel.innerHTML = `<span class="text-red-500 font-bold">⚠️ Connection issue</span> <button onclick="loadProducts()" class="underline font-bold ml-2 text-amber-600">Tap to Retry</button>`;
    }
    if (grid && allProducts.length === 0) {
      grid.innerHTML = `
        <div class="col-span-full py-12 text-center text-slate-500 bg-white rounded-2xl border border-red-200 p-6">
          <div class="w-12 h-12 rounded-2xl bg-red-50 text-red-600 flex items-center justify-center mx-auto mb-3 text-xl font-bold">⚠️</div>
          <h3 class="text-sm font-bold text-slate-800">Store items could not be loaded</h3>
          <p class="text-xs text-slate-500 mt-1 mb-4">Please check your internet connection or tap retry below.</p>
          <button onclick="loadProducts()" class="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-bold rounded-xl shadow transition">
            🔄 Retry Loading
          </button>
        </div>
      `;
...
function renderCustomerStore() {
  ...
  const filtered = allProducts.filter(p => {
    ...
    const matchesCat = (selectedCustomerCategory === 'All' || (p.category || 'General') === selectedCustomerCategory);
...
function renderAdminInventory() {
  ...
  const filtered = allProducts.filter(p => {
    ...
    const pCat = (p.category || 'General').toLowerCase();
    const matchesSearch = pName.includes(search) || pCat.includes(search);
    const matchesCat = (cat === 'All' || (p.category || 'General') === cat);
    return matchesSearch && matchesCat;
  });
